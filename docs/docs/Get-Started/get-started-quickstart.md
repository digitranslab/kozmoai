...
Get to know Kozmoai by building an OpenAI-powered chatbot application. After you've constructed a chatbot, add Retrieval Augmented Generation (RAG) to chat with your own data.
...
## Open Kozmoai and start a new project

1. From the Kozmoai dashboard, click **New Flow**, and then select **Blank Flow**. A blank workspace opens where you can build your flow.
...
1. Add your credentials to the OpenAI component. The fastest way to complete these fields is with Kozmoai’s [Global Variables](/configuration-global-variables).
...
You have successfully created a chatbot application using OpenAI in the Kozmoai Workspace.

## Add vector RAG to your application

You created a chatbot application with Kozmoai, but let's try an experiment.
...
If you used Kozmoai's **Global Variables** feature, the RAG application flow components are already configured with the necessary credentials.
