...
The **Python Code** tab displays code to interact with your flow's `.json` file using the Kozmoai runtime.

To use your code in a Python application using the Kozmoai runtime, you have to first download your flow’s JSON file.
...
```python
from kozmoai.load import run_flow_from_json
TWEAKS = {
  "ChatInput-kKhri": {},
  "Prompt-KDSi5": {},
  "ChatOutput-Vr3Q7": {},
...
For information on sending files to the Kozmoai API, see [API examples](/api-reference-api-examples#upload-image-files).

## Chat Widget

The **Chat Widget HTML** tab displays code that can be inserted in the `<body>` of your HTML to interact with your flow.

The **Kozmoai Chat Widget** is a powerful web component that enables communication with a Kozmoai project. This widget allows for a chat interface embedding, allowing the integration of Kozmoai into web applications effortlessly.
...
```html
<script src="https://cdn.jsdelivr.net/gh/logspace-ai/kozmoai-embedded-chat@v1.0.7/dist/build/static/js/bundle.min.js""></script>
...
```javascript
<script src="https://cdn.jsdelivr.net/gh/digitranslab/kozmoai-embedded-chat@main/dist/build/static/js/bundle.min.js"></script>
```
...
- Open the module file (it typically ends with _.module.ts_) where you'd add the `kozmoai-chat` web component.
...
In your Angular project, find the component belonging to the module where `CUSTOM_ELEMENTS_SCHEMA` was added. Inside the template, add the `kozmoai-chat` tag to include the Chat Widget in your component's view:

```javascript
<kozmoai-chat  chat_inputs='{"your_key":"value"}'  chat_input_field="your_chat_key"  flow_id="your_flow_id"  host_url="kozmoai_url"></kozmoai-chat>
```

:::tip

`CUSTOM_ELEMENTS_SCHEMA` is a built-in schema that allows Angular to recognize custom elements. Adding `CUSTOM_ELEMENTS_SCHEMA` tells Angular to allow custom elements in your templates, and it will suppress the warning related to unknown elements like `kozmoai-chat`. Notice that you can only use the Chat Widget in components that are part of the module where you added `CUSTOM_ELEMENTS_SCHEMA`.
