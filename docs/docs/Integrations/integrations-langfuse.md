...
# Integrate Langfuse with Kozmoai
...
This guide walks you through how to configure Kozmoai to collect [tracing](https://langfuse.com/docs/tracing) data about your flow executions and automatically send the data to Langfuse.

## Prerequisites

- A project in Kozmoai with a runnable flow
...
Set your Langfuse project credentials as environment variables in the same environment where you run Kozmoai.
...
## Start Kozmoai and run a flow

1. Start Kozmoai in the same terminal or environment where you set the environment variables:

    ```bash
    python -m kozmoai run
    ```

2. In Kozmoai, open and existing project, and then run a flow.

## View tracing data in Langfuse

Kozmoai automatically collects and sends tracing data about the flow execution to Langfuse.
You can view the collected data in your Langfuse project dashboard.

## Disable the Langfuse integration

To disable the Langfuse integration, remove the environment variables you set in the previous steps and restart Kozmoai.
