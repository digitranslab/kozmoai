...
Kozmoai uses anonymous telemetry to collect essential usage statistics to enhance functionality and the user experience. This data helps us identify popular features and areas that need improvement, and ensures development efforts align with what you need.

We respect your privacy and are committed to protecting your data. We do not collect any personal information or sensitive data. All telemetry data is anonymized and used solely for improving Kozmoai.

## Opt out of telemetry

To opt out of telemetry, set the `KOZMOAI_DO_NOT_TRACK` or `DO_NOT_TRACK` environment variable to `true` before running Kozmoai. This disables telemetry data collection.

## Data that Kozmoai collects
...
- **Version**: The specific version of Kozmoai used, which helps in tracking feature adoption and compatibility.
...
- **BackendOnly**: Boolean indicating whether you are running Kozmoai in a backend-only mode, useful for understanding deployment configurations.
