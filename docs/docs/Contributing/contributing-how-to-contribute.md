---
title: Contribute to Kozmoai
slug: /contributing-how-to-contribute
---

This guide is intended to help you start contributing to Kozmoai.
As an open-source project in a rapidly developing field, Kozmoai welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.
...
Develop Kozmoai locally with [uv](https://docs.astral.sh/uv/getting-started/installation/) and [Node.js](https://nodejs.org/en/download/package-manager).
...
### Clone the Kozmoai Repository

1. Navigate to the [Kozmoai GitHub repository](https://github.com/digitranslab/kozmoai), and then click **Fork**.

2. Add the new remote to your local repository on your local machine:

```bash
git remote add fork https://github.com/<your_git_username>/kozmoai.git
```
...
This command sets up the development environment by installing backend and frontend dependencies, building the frontend static files, and initializing the project. It runs `make install_backend`, `make install_frontend`, `make build_frontend`, and finally `uv run kozmoai run` to start the application.

2. Run `make lint`, `make format`, and `make unit_tests` before pushing to the repository.

### Debug

The repo includes a `.vscode/launch.json` file for debugging the backend in VSCode, which is faster than debugging with Docker Compose. To debug Kozmoai with the `launch.json` file in VSCode:

1. Open Kozmoai in VSCode.
...
After setting up the environment with `make init`, you can run Kozmoai's backend and frontend separately for development.
Kozmoai recommends using a virtual environment like [venv](https://docs.python.org/3/library/venv.html) or [conda](https://anaconda.org/anaconda/conda) to isolate dependencies.
...
Once you have written and manually tested your changes with `make lint` and `make unit_tests`, open a pull request to send your changes upstream to the main Kozmoai repository.
...
3. A Kozmoai maintainer will review your pull request. Thanks for your contribution!
...
* Pull request titles appear in Kozmoai's release notes, so they should explain what the PR does as explicitly as possible.
