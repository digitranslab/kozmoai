...
Chat memory in Kozmoai is stored either in local Kozmoai tables with `LCBufferMemory`, or connected to an external database.
...
The Current Date component returns the current date and time in a selected timezone. This component provides a flexible way to obtain timezone-specific date and time information within a Kozmoai pipeline.
...
:::info
Prior to Kozmoai 1.1, this component was known as the Chat Memory component.
:::

This component retrieves and manages chat messages from Kozmoai tables or an external memory.

### Inputs

| Name | Display Name | Info |
|------|--------------|------|
| memory | External Memory | Retrieve messages from an external memory. If empty, it will use the Kozmoai tables. |
...
This component stores chat messages or text into Kozmoai tables or an external memory.
...
| memory | External Memory | The external memory to store the message. If empty, it will use the Kozmoai tables. |
