...
You will push changes to a fork of the Kozmoai repository, and from there create a Pull Request into the project repository.

Fork the [Kozmoai GitHub repository](https://github.com/digitranslab/kozmoai/fork), and follow the instructions to create a new fork.
...
```bash
git clone https://github.com/<your username>/kozmoai.git
```
...
```bash
cd kozmoai
git remote add upstream https://github.com/digitranslab/kozmoai.git
...
This will set up the development environment by installing backend and frontend dependencies, building the frontend static files, and initializing the project. It runs `make install_backend`, `make install_frontend`, `make build_frontend`, and finally `uv run kozmoai run` to start the application.
...
However, as a developer,  you will want to proceed to the next step. Shutdown Kozmoai by hitting `Control (or Command)-C`.
...
## Run Kozmoai in "Development" Mode
...
```
INFO:     Will watch for changes in these directories: ['/home/phil/git/kozmoai']
...
The frontend (User Interface) is, in shipped code (i.e. via `kozmoai run`), statically-compiled files that the backend FastAPI service provides to clients via port `7860`. In development mode, these are served by a Node.js service on port `3000`. In the *Frontend Terminal*, start the frontend service:
...
At this point, you can navigate to http://localhost:3000/ in a browser and access the Kozmoai User Interface.
...
Components reside in folders under `src/backend/base/kozmoai`, and their unit tests under `src/backend/base/tests/unit/components`. 
...
As a final validation, stop the backend and frontend services and run `make init`; this will do a clean build and the UI should be available in port `7860` (as it has invoked `kozmoai run`). Open a **new** browser tab to this  service and do a final check of your changes by adding your new/modified component onto the canvas from the Components list.
...
* Files in `src/backend/base/kozmoai/initial_setup/starter_projects` modify after `kozmoai run`; these are formatting changes. Feel free to commit (or ignore) them.
